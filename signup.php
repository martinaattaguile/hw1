<?php
        ...
        !empty($_POST["surname"]) && !empty($_POST["confirm_password"]) && !empty($_POST["allow"]))
    {
        ...
        if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            if (mysqli_num_rows($res) > 0) {
                $error[] = "Indirizzo email già utilizzato";
            }
        }
        ...
        if (count($error) == 0) {
            ...
            if (mysqli_query($conn, $query)) {
                $_SESSION["_YourEvents_username"] = $_POST["username"];
                $_SESSION["_YourEvents_user_id"] = mysqli_insert_id($conn);
                mysqli_close($conn);
                header("Location: home.php");
                exit;
            } else {
        ...
        <main>
            ...
            <form name='signup' method='post' enctype="multipart/form-data" autocomplete="off">
                ...
                <div class="fileupload">
                    <div><label for='avatar'>Seleziona l'immagine del profilo</label></div>
                    <div>
                        <input type='file' name='avatar' accept='.jpg, .jpeg, image/gif, image/png' id="upload_original">
                        <div id="upload"><div class="file_name">Seleziona un file</div></div>
                    </div>
                </div>
                <div class="allow"> 
            ...
            <div class="signup">Hai un account? <a href="login.php"><strong>Accedi</strong></a>
        </section>
        </main>
    </body>
</html>
